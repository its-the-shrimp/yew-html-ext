...
fn compile_pass() {
    ...
    _ = html! { <div>{children}</div> };

    // handle misleading angle brackets
    _ = html! { <div data-val={<::std::string::String as ::std::default::Default>::default()}></div> };
    _ = html! { <div><a data-val={<::std::string::String as ::std::default::Default>::default()} /></div> };

    // test for https://github.com/yewstack/yew/issues/2810
    _ = html! {  <div data-type="date" data-as="calender" /> };

    let option_vnode = ::std::option::Option::Some(html! {});
    _ = html! { <div>{option_vnode}</div> };
}
